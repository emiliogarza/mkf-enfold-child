...
/**
 * Custom Modal
 * Shortcode which creates a simple text modal
 */

if ( !class_exists( 'avia_sc_custom_modal' ) )
{
	class avia_sc_custom_modal extends aviaShortcodeTemplate
	{
			/**
			...
			{
				$this->config['name']			= __('Custom Modal', 'avia_framework' );
				$this->config['tab']			= __('Content Elements', 'avia_framework' );
				$this->config['icon']			= AviaBuilder::$path['imagesURL']."sc-fullscreen.png";
				$this->config['order']			= 100;
				$this->config['target']			= 'avia-target-insert';
				$this->config['shortcode'] 		= 'av_custommodal';
				$this->config['tinyMCE'] 	    = array('disable' => true);
				$this->config['tooltip'] 	    = __('Creates a simple text modal', 'avia_framework' );
			...
			function popup_elements()
			{
				$this->elements = array(
                    ...
                    "std" => ""),
          array(
            "desc" 	=> __("Would you like to retain the user's dismissal and not show the popup again?", 'avia_framework'),
            "id" 	=> "cookieDismissal",
            "std" 	=> "",
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
                ...
                $output .= '<div id="custom-popup" class="white-popup mfp-hide '.$av_display_classes.'" '.$markup.' style="position: relative; background: #FFF; padding: 20px; width: auto; max-width: 500px; margin: 20px auto;">';
                ...
                $output .= '</div>';
                $output .= '<script>';
                $output .= '$(document).ready(function(){$.magnificPopup.open({items:{src:"#custom-popup",type:"inline"}})});';
                $output .= '</script>';
                return $output;
			}
